...
_SUCCESS_TTL = 300  # 성공 상태 5분 캐시
_ERROR_TTL = 30     # 실패 상태는 30초만 캐시 (일시 SSL 오류 잔존 방지)


def _request_with_retry(method: str, url: str, **kwargs) -> requests.Response:
    """SSL/Connection 일시 오류에 대해 1회 자동 재시도. requests.RequestException은 그대로 raise."""
    try:
        return requests.request(method, url, **kwargs)
    except (requests.exceptions.SSLError, requests.exceptions.ConnectionError) as e:
        _log.info(f"일시 네트워크 오류, 재시도: {type(e).__name__}")
        time.sleep(0.3)
        return requests.request(method, url, **kwargs)
...
# ---------------------------------------------------------------------------
# 통합 점검 (성공/실패별 가변 TTL 캐시 + 병렬 실행)
# ---------------------------------------------------------------------------
_STATUS_CACHE: dict = {"data": None, "ts": 0.0, "ttl": 0}


def _run_all_checks_parallel() -> Dict[str, Tuple[str, str]]:
    """Gemini / Google Search / Claude ping을 병렬 실행. 총 대기 시간 ≈ max(개별) ≤ 6s."""
    with ThreadPoolExecutor(max_workers=3) as ex:
        f_gemini = ex.submit(_check_gemini)
        f_gsearch = ex.submit(_check_google_search)
        f_claude = ex.submit(_check_claude)
        gemini_state = f_gemini.result()
        gsearch_state = f_gsearch.result()
        claude_state = f_claude.result()
    return {
        "Gemini Pro": gemini_state,
        "Gemini Flash": gemini_state,
        "Google Search": gsearch_state,
        "Claude": claude_state,
    }
...
def check_all_apis() -> Dict[str, Tuple[str, str]]:
    ...
    - 모든 항목이 ok 이면 5분(_SUCCESS_TTL) 캐시.
    - 하나라도 error/warn 이면 30초(_ERROR_TTL) 캐시 → 일시 SSL/네트워크 오류가 5분간
      잔존하는 문제(페이지 전환 후 모든 API 빨강) 방지.
    """
    now = time.time()
    cached = _STATUS_CACHE.get("data")
    if cached and (now - _STATUS_CACHE["ts"] < _STATUS_CACHE["ttl"]):
        return cached
    data = _run_all_checks_parallel()
    all_ok = all(state == "ok" for state, _ in data.values())
    _STATUS_CACHE["data"] = data
    _STATUS_CACHE["ts"] = now
    _STATUS_CACHE["ttl"] = _SUCCESS_TTL if all_ok else _ERROR_TTL
    return data
